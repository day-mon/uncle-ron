...
from discord.ext.commands import Cog, Bot, hybrid_group, Context, group
...
class Links(Cog):
    ...
    async def on_message(self, message: discord.Message):
        ...
        for url in urls:
            try:
                ...
                self.logger.debug(f"Found URL: {url} from {message.author.name} in {guild_name}")
                ...
                self.logger.info(f"Stored link from {hostname} by user {message.author.name} in {guild_name}")
    ...
    async def links_stats(self, ctx: Context):
        self.logger.info(f"User {ctx.author.name} requested link stats in guild {ctx.guild.name}")
    ...
    async def links_my(self, ctx: Context):
        self.logger.info(f"User {ctx.author.name} requested personal link stats in guild {ctx.guild.name}")
        embed = await self._create_user_stats_embed(ctx.guild.id, ctx.author.id)
        await ctx.send(embed=embed)
        self.logger.debug(f"Sent personal link stats to {ctx.author.name} in {ctx.guild.name}")
    ...
    async def _create_user_stats_embed(self, guild_id: int, user_id: int) -> discord.Embed:
